 ...
 * Enables auto-invocation for amplified development workflows
 */
  ...
  try {
    ...
    mdFiles.forEach(file => {
      try {
        ...
        console.log(`[Ampcode] Registered agent: ${metadata.id} - ${metadata.name}`);
      } catch (error) {
...
/**
 * Auto-invocation handler for amplified workflows
 * Matches user commands to available agents and invokes them
 * Optimized for velocity and automation
 */
function createAutoInvocationHandler(registry) {
  return function invokeAgent(agentId, context) {
    ...
    // Amplified mode: fast execution, automation-ready
    return {
      agentId: agent.id,
      name: agent.name,
