/**
 * register-agents.js
 * Auto-discovery mechanism that scans agent/ directory (Opencode uses singular 'agent')
 * Extracts metadata from agent files
 * Registers all agents in context.agentRegistry
 * Enables auto-invocation for CLI-based workflows
 */
  ...
  try {
    ...
    mdFiles.forEach(file => {
      try {
        ...
        console.log(`[Opencode] Registered agent: ${metadata.id} - ${metadata.name}`);
      } catch (error) {
    ...
    console.error(`Failed to scan agent directory: ${error.message}`);
  }
...
/**
 * Auto-invocation handler for CLI workflows
 * Matches user commands to available agents and invokes them
 * Optimized for terminal-based interactions
 */
function createAutoInvocationHandler(registry) {
  return function invokeAgent(agentId, context) {
    ...
    // CLI-optimized: minimal output, command-line friendly
    return {
      agentId: agent.id,
      name: agent.name,
