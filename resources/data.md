# Data Files Library
Consolidated on: 2025-11-01T17:48:47.228307
This file consolidates all data reference files.
## Table of Contents
...
Agentic Kit Method (Breakthrough Method of Agile AI-driven Development) is a framework that combines AI agents with Agile development methodologies. The v4 system introduces a modular architecture with improved dependency management, bundle optimization, and support for both web and IDE environments.
...
**Note for VS Code Users**: Agentic Kit Method assumes when you mention "VS Code" that you're using it with an AI-powered extension like GitHub Copilot, Cline, or Roo. Standard VS Code without AI capabilities cannot run Agentic Kit agents. The installer includes built-in support for Cline and Roo.

**Verify Installation**:

- `.` folder created with all agents
- IDE-specific integration files created
- All agent commands/rules/modes available

**Remember**: At its core, Agentic Kit Method is about mastering and harnessing prompt engineering. Any IDE with AI agent support can use Agentic Kit - the framework provides the structured prompts and workflows that make AI development effective
...
**New in V4**: The `.core-config.yaml` file is a critical innovation that enables Agentic Kit to work seamlessly with any project structure, providing maximum flexibility and backwards compatibility.

### What is core-config.yaml?

This configuration file acts as a map for Agentic Kit agents, telling them exactly where to find your project documents and how they're structured. It enables:
...
3. **Custom Workflows**: Configure Agentic Kit to match your team's process
...
| Agent               | Role             | Primary Functions                     | When to Use                       |
| ------------------- | ---------------- | ------------------------------------- | --------------------------------- |
| `kit-orchestrator` | Team Coordinator | Multi-agent workflows, role switching | Complex multi-role tasks          |
| `kit-master`       | Universal Expert | All capabilities without switching    | Single-session comprehensive work |
...
- **Opencode**: `@agent-name` (e.g., `@master`)
- **Cursor**: `@agent-name` (e.g., `@master`)
- **Windsurf**: `/agent-name` (e.g., `/master`)
- **Trae**: `@agent-name` (e.g., `@master`)
- **Roo Code**: Select mode from mode selector (e.g., `kit-master`)
...
The Agentic Kit Method is built around a modular architecture centered on the `agentic-kit` directory, which serves as the brain of the entire system. This design enables the framework to operate effectively in both IDE environments (like Cursor, VS Code) and web-based AI interfaces (like ChatGPT, Gemini).
...
- **Templates** (`templates/`): Markdown templates for PRDs, architecture specs, user stories
- **Tasks** (`tasks/`): Instructions for specific repeatable actions like "shard-doc" or "create-next-story"
- **Checklists** (`checklists/`): Quality assurance checklists for validation and review
- **Data** (`data/`): Core knowledge base and technical preferences
...
Agentic Kit employs a sophisticated template system with three key components:

1. **Template Format** (`utils/doc-template.md`): Defines markup language for variable substitution and AI processing directives from yaml templates
...
This architecture enables seamless operation across environments while maintaining the rich, interconnected agent ecosystem that makes Agentic Kit powerful.
...
1. **Document Sharding** (CRITICAL STEP):
   ...
   - Two methods to shard:
     ...
     b) **Agent**: Ask `@kit-master` or `@po` to shard documents
...
- Use specific agents vs. `kit-master` for focused tasks
...
## Contributing to Agentic Kit Method
...
Expansion packs extend Agentic Kit Method beyond traditional software development into ANY domain. They provide specialized agent teams, templates, and workflows while keeping the core framework lean and focused on development.
